...
async function carregar(){
    ...
    if(filtroMes){
        dadosFiltrados = dadosFiltrados.filter(l => l.data.startsWith(filtroMes))
    }
    if(filtroOrgao){
        dadosFiltrados = dadosFiltrados.filter(l => l.orgao.toLowerCase().includes(filtroOrgao))
    }
    if(filtroEstado){
        dadosFiltrados = dadosFiltrados.filter(l => l.estado === filtroEstado)
    }

    let total=dadosFiltrados.length, andamento=0, ganhas=0, perdidas=0
    ...
    let licHoje = dadosFiltrados.filter(l => l.data === hoje)
        ...
        <div class="col-lg-3 col-md-4 col-sm-6 mb-2">
            <div class="card p-2 shadow-sm ${destaque}" onclick="editar(${l.id})">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <strong>${l.orgao}</strong><br>
                        <small class="text-muted">${l.servico}</small>
                    </div>
